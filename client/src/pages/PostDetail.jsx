    ...
    try {
      if (navigator.share) {
        await navigator.share({
          title: `Post by ${post?.user?.name} on Visionex`,
        ...
        <div className="p-6 md:p-8 flex flex-col gap-8 h-full">
          ...
          <div className="relative flex items-center justify-between">
              ...
              modal && (
                <div className="p-8 h-52 md:w-80 w-100 bg-gray-900 rounded-lg absolute top-12 right-5 z-50 border border-white/10 shadow-2xl">
                  <form onSubmit={handleReportPost}>
                    ...
                    <button type='submit' className="mt-4 w-full cursor-pointer py-2 rounded-xl flex items-center justify-center gap-2 font-bold transition-all duration-300 bg-white/5 border border-red-500/30 text-red-300 hover:bg-red-600/20">Submit Report</button>
                  </form>
                </div>
              )
            }
            ...
            <div className="flex gap-3">
              <button onClick={handleShare} className="w-10 h-10 rounded-full glass-card flex items-center justify-center hover:bg-white/10 transition-colors">
              ...
              >
                <Bookmark className={`w-4 h-4 ${isSaved ? 'text-orange-500 fill-orange-500' : 'text-gray-300'}`} />
              </button>
            </div>
          ...
          <div className="flex items-center justify-between">
            <Link to={`/auth/profile/${post?.user?.name}`} className="flex items-center gap-3 group">
              ...
              <div>
                <p className="font-bold text-white group-hover:text-red-400 transition-colors">{post?.user?.name}</p>
                <p className="text-xs text-gray-400">{post.user.followers.length} followers</p>
              </div>
            </Link>
            {post?.user?._id !== (user?.id || user?._id) && (
              <button onClick={() => handleFollowUnfollow(post.user._id)} className={alreadyFollowed ? "px-5 py-2 rounded-full text-white font-medium hover:bg-white/20 transition-colors text-sm bg-red-500" : "px-5 py-2 rounded-full bg-white/10 text-white font-medium hover:bg-white/20 transition-colors text-sm"}>
          ...
          <div className="flex-1 mt-4">
            ...
            <div className="bg-black/40 border border-white/5 rounded-xl p-5 relative group">
              <p className="text-gray-300 leading-relaxed font-mono text-sm selection:bg-red-500/30">
                {post.prompt}
              </p>
              <button className="absolute top-2 right-2 p-2 rounded bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity hover:bg-white/20 text-xs font-semibold text-red-300">
                Copy
              </button>
            </div>

            <button className="mt-6 w-full py-4 rounded-xl flex items-center justify-center gap-2 font-bold transition-all duration-300 bg-white/5 border border-red-500/30 text-red-300 hover:bg-red-600/20">
